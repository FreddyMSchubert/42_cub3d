...
/*   Updated: 2024/06/25 18:44:58 by freddy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_player(bool *invalid)
{
	if (*invalid)
		return (false);
	if ((player(0)->spawn_transform.rot.x == -1
		&& player(0)->spawn_transform.rot.y == -1) || 
		(player(1)->spawn_transform.rot.x == -1
		&& player(1)->spawn_transform.rot.y == -1))
	{
		*invalid = true;
		logger(LOGGER_ERROR, "Player spawn look dir not set!");
	}
	if ((player(0)->spawn_transform.pos.x == -1
		&& player(0)->spawn_transform.pos.y == -1) ||
		(player(1)->spawn_transform.pos.x == -1
		&& player(1)->spawn_transform.pos.y == -1))
	{
		*invalid = true;
