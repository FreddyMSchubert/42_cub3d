/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   1_raycast_walls.c                                  :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: freddy <freddy@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/30 13:56:58 by freddy            #+#    #+#             */
/*   Updated: 2024/06/25 20:08:18 by freddy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../../include/cub3d.h"

double	get_fisheye_corrected_ray_angle(int ray_index)
{
	double	relative_position;
	double	angle_correction;

	relative_position = ((double)ray_index / ((RAYCASTS_PER_DEG * FOV_DEG) / 2)) - 1;
	angle_correction = atan(relative_position * tan(deg_to_rad(FOV_DEG / 2)));
	return (rad_to_deg(angle_correction));
}

static t_entity	*perform_entity_raycast(int i, t_vec2 *entity_intersection, int id)
{
	t_transform	ray;
	double		p_angle;
	double		ray_angle;
	t_entity	*entity;

	p_angle = dir_vec_to_deg(player(id)->transform.rot);
	ray_angle = get_fisheye_corrected_ray_angle(i);
	ray_angle += p_angle;
	ray.rot = deg_to_dir_vec(ray_angle);
	ray.rot = scale_vector(ray.rot, VIEW_DIST);
	ray.pos = player(id)->transform.pos;
	*entity_intersection = get_entity_intersection(game()->entities, ray, &entity, id);
	return (entity);
}

static inline t_vec2	perform_wall_raycast(int ray_index, int id)
{
	t_transform	ray;
	double		p_angle_deg;
	double		angle_correction;
	double		ray_angle_deg;

	p_angle_deg = dir_vec_to_deg(player(id)->transform.rot);
	angle_correction = get_fisheye_corrected_ray_angle(ray_index);
	ray_angle_deg = p_angle_deg + angle_correction;
	ray.rot = deg_to_dir_vec(ray_angle_deg);
	ray.rot = scale_vector(ray.rot, VIEW_DIST);
	ray.pos = player(id)->transform.pos;
	return (get_wall_intersection(game()->input_data->walls, ray));
}

void	raycast_walls(int id)
{
	int			ray_index;
	t_vec2		wall_intersect;
	t_vec2		entity_intersect;
	t_entity	*ntt;

	ray_index = -1;
	while (++ray_index < RAYCASTS_PER_DEG * FOV_DEG)
	{
		wall_intersect = perform_wall_raycast(ray_index, id);
		ntt = perform_entity_raycast(ray_index, &entity_intersect, id);
		calc_wall(ray_index, wall_intersect, id);
		if (ntt && entity_intersect.x != -1 && entity_intersect.y != -1 && \
				(pos_distance(player(id)->transform.pos, entity_intersect) < pos_distance(player(id)->transform.pos, wall_intersect) || RENDER_ENTITIES_THROUGH_WALLS))
			calc_entity(ray_index, entity_intersect, ntt, id);
	}
}
