...
/*   Updated: 2024/06/25 20:11:15 by freddy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_mouse_mv(void)
{
	...
	if (game()->player_count > 1)
		imp_player = 1;
	mlx_get_mouse_pos(player(imp_player)->mlx, &current_mouse_x, &current_mouse_y);
	if (current_mouse_x == game()->prev_mouse_x)
		return ;
	player(imp_player)->dirty = true;
	if (MARK_DIRTY_LOGGING)
		logger(LOGGER_INFO, "Mouse moved, set dirty to true!\n");
	turn((current_mouse_x - game()->prev_mouse_x) * MOUSE_SENSITIVITY, imp_player);
